...
function Game(props) {
  ...
  const addCurrent = () => {
    if (check === props.countryName || check === props.countryEnName) {
      alert("정답");
      props.addCurrent(add);
      setAdd(100);
      setHint1(true);
      setHint2(true);
      setHint3(true);
  ...
  return (
    <div id="game">
      ...
      <div className="game-check">
        ...
        <button className="btn-check" onClick={addCurrent}>
          정답!
        </button>
        <button className="btn-pass" onClick={pass}>
          패스
        </button>
        <div className="game-score">점수 : {props.score}</div>
      </div>

      <div className="btn-hints">
        {hint1 ? (
          <button onClick={addhint1}>힌트1</button>
        ) : (
          <div className="game-hint1">수도 : {capital}</div>
        )}
        {hint2 ? (
          <button onClick={addhint2}>힌트2</button>
        ) : (
          <div className="game-hint2">
            대륙 : {props.continent} <br /> 면적: {area}
          </div>
        )}
        {hint3 ? (
          <button onClick={addhint3}>힌트3</button>
        ) : (
          <div className="game-hint3">
            종교 : {religion} <br /> 언어 : {language}
          </div>
        )}
      </div>
    </div>
